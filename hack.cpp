    ...
    { //if we have an alive target...
        ...
        { //if we have a target and we are still clicking alt2, or if we are in the process of moving to a target...
            ...
            if ((xhairDistance <= fov || fov == 0) && xhairDistance != -1.0)
            {
                ...
                {
                    helper::Smoothing(&viewAngle, &newAngle, 0, percentSmoothing / 20, hack::errormargin); //reduce smoothing because we are already on target
                    shouldShoot = true;
                }
                else
                {
                    helper::Smoothing(&viewAngle, &newAngle, hack::willsalt, percentSmoothing, hack::errormargin);
                    if(hack::flickshoot && !helper::ShouldNotRCS(iWeaponID_lp)) {
                        shouldShoot = true;
                        alreadyShotOnce = false;
                    }
                }
    ...
    for (unsigned int i = 0; i < count; i++)
    {
        if (g_glow[i].m_pEntity != NULL)
        {
            ...
            if (csgo.Read(g_glow[i].m_pEntity, &ent, sizeof(Entity)))
            {
                ...
                spottedByMe = (spottedByMask>>local.ID-1)&1;
                if (ent.m_iTeamNum == 2 || ent.m_iTeamNum == 3)
                {
                ...
                for (unsigned j = 0, test = 1; j < 8; ++j, test *= 2)
                {
                        ...
                        ((keys[i] & test) != (lastkeys[i] & test)))
                    {
                        ...
                        else if (code == keycodeTrigger)
                        {
                            ...
                            cout << "Trigger: " << ShouldTrigger << endl;
                        }
...
void hack::init()
{
    cout << "CS:GO external hack for linux" << endl << "Fork of JWaim by Jalius Welch" << endl;
    if (getuid() != 0)
    {
    ...
    hack::ShouldTriggerToggleKey = helper::getConfigValue("ToggleTrigger", cfg);
    keycodeTrigger = XKeysymToKeycode(hack::display, XStringToKeysym(hack::ShouldTriggerToggleKey.c_str()));
    cout << "Trigger Toggle = " << hack::ShouldTriggerToggleKey.c_str() << " Keycode: " << keycodeTrigger << endl;
    ...
    hack::flickshoot = ::atof(helper::getConfigValue("aim_flickshoot", cfg).c_str());
